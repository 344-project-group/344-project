...
{#if $modalStore[0]}
    <div class="modal-example-form {cBase}">
        <header class={cHeader}>{$modalStore[0].title ?? 'Create Task'}</header>
        ...
        <form class="modal-form {cForm}">
            ...
            <label class="label">
                <span>Duration</span>
                <input class="input" type="number" bind:value={formData.duration} placeholder="Enter duration..." />
            </label>
            <label class="label">
                <span>Min. chunk</span>
                <input class="input" type="number" bind:value={formData.minChunk} placeholder="Enter min. chunk..." />
            </label>
            <label class="label">
                <span>Labels</span>
                <select multiple class="select" bind:value={formData.labels} on:change={handleLabelSelection}>
            ...
            {#if formData.startDate}
                {#each Object.keys(formData.schedule) as day}
                    <div>
                        <input type="checkbox" checked={formData.selectedDay === day} on:change={() => handleDaySelection(day)} />
                        <span>{day}</span>
                    </div>
                {/each}
            {/if}
            ...
            {#if formData.selectedDay !== null}
                <div>
                    {#each Array.from(Array(24).keys()) as hour}
                        <input type="checkbox" checked={formData.selectedHours.includes(hour)} on:change={() => handleHourSelection(hour)} />
                        <span>{hour}:00</span>
                    {/each}
                </div>
            {/if}
